...
const nav: NavEntry[] = [
  ...
  },
  {
    ...
    icon: GraduationCap,
    children: [
      { to: "/admin/instructors", label: "المعلمون", icon: GraduationCap },
      { to: "/admin/withdrawals", label: "سحوبات المعلمين", icon: Landmark },
      { to: "/admin/revenue-settings", label: "تقسيم الأرباح", icon: Coins },
    ],
  },
  {
    ...
    icon: Settings2,
    children: [
      { to: "/admin/settings", label: "الإعدادات العامة", icon: Settings2, end: true },
      { to: "/admin/settings/video-player", label: "مشغل الفيديو", icon: PlayCircle },
      { to: "/admin/settings/registration-form", label: "نموذج التسجيل", icon: FileText },
      { to: "/admin/settings/student-qr", label: "QR الطلاب", icon: CreditCard },
      { to: "/admin/settings/branding", label: "الهوية البصرية", icon: Sliders },
    ...
    <div className="flex h-full flex-col">
      <div className={`${collapsed ? "px-3" : "px-6"} py-6 border-b border-border/60`}>
        <NavLink to="/" className={`flex items-center ${collapsed ? "justify-center" : "gap-3"} group`}>
          <img src="/motivai-logo.png" alt="شعار Motivai" className="h-11 w-11 rounded-lg object-contain" />
          {!collapsed && (
            <div>
