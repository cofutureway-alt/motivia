...
const whatsappDb = supabase as any;
...
export async function fetchWhatsappSettings(): Promise<{
  ...
  const [settingsRes, secretsRes] = await Promise.all([
    (supabase as any).from("whatsapp_settings").select("*").eq("id", 1).maybeSingle(),
    (supabase as any).from("whatsapp_secrets").select("*").eq("id", 1).maybeSingle(),
  ]);
...
export async function fetchWhatsappInstances(): Promise<WhatsappInstance[]> {
  const { data, error } = await whatsappDb
...
export async function deleteWhatsappInstance(id: string) {
  const { error } = await (supabase as any).from("whatsapp_instances").delete().eq("id", id);
  if (error) throw error;
}
...
export async function triggerWhatsappDispatcher(): Promise<{
  ...
  const { data, error } = await (supabase as any).rpc("admin_trigger_whatsapp_dispatcher");
  if (error) throw error;
  return data as any;
}
