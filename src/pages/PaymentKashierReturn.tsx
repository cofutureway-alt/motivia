...
export default function PaymentKashierReturn() {
  ...
  const reference = params.get("merchantOrderId") || params.get("orderId") || params.get("reference") || "";
  const kashierPaymentStatus = (params.get("paymentStatus") || "").toUpperCase();
  ...
  const amount = txn?.purpose === "wallet_topup" ? txn.topup_amount_piastres ?? 0 : txn?.amount_piastres ?? 0;
      ...
      >
        <div className="p-8 space-y-6">
          <div className="flex items-center gap-2 text-xs text-muted-foreground">
            ...
            <span>عودة آمنة من بوابة Kashier</span>
          </div>
              ...
              >
                ...
                <div>
                  ...
                  <div className="text-sm text-muted-foreground mt-2 leading-relaxed">
                    نتحقق من نتيجة عملية الدفع مع Kashier. عادةً ما يستغرق ذلك بضع ثوانٍ.
                  </div>
                </div>
            ...
            {state === "pending" && (
              ...
              >
                ...
                <div>
                  ...
                  <div className="text-sm text-muted-foreground mt-2 leading-relaxed">
                    استلمنا عودتك من صفحة الدفع، لكن لم يصلنا تأكيد Kashier النهائي بعد. سنُحدّث الحالة تلقائيًا فور استلامه.
                  </div>
                  {kashierPaymentStatus && (
                    <div className="mt-3 inline-flex text-[11px] font-mono bg-accent px-2 py-1 rounded">
                      {kashierPaymentStatus}
                    </div>
                  )}
                </div>
                <ActionRow onGo={() => navigate("/dashboard/wallet")} label="متابعة طلباتي" />
              </motion.div>
            )}
...
function ActionRow({ onGo, label }: { onGo: () => void; label: string }) {
  return (
