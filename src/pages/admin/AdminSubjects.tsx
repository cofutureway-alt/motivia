...
import { Plus, BookMarked, Search, Sparkles, AlertTriangle } from "lucide-react";
...
import SubjectFormModal, { type SubjectRow } from "@/components/admin/SubjectFormModal";
import SubjectCard from "@/components/admin/SubjectCard";

type SubjectWithCount = SubjectRow & { courses_count: number };
...
const AdminSubjects = () => {
  ...
  const [deleting, setDeleting] = useState<SubjectWithCount | null>(null);
  ...
  const confirmDelete = async () => {
    ...
    if (deleting.courses_count > 0) {
      toast.error(
        "لا يمكن حذف هذه المادة لأنها مرتبطة بدورات. يجب إعادة تعيين المادة على تلك الدورات أولًا.",
      );
      setDeleting(null);
      return;
    }
    ...
    try {
      ...
      toast.error(e?.message || "تعذّر حذف المادة");
    } finally {
    ...
    <div className="max-w-7xl mx-auto">
      ...
      >
        <div>
          ...
          <p className="text-muted-foreground mt-2">
            نظّم دوراتك عبر مواد دراسية واضحة.
          </p>
        </div>
      ...
      {subjects === null ? (
        ...
        >
          ...
          <h2 className="text-2xl font-bold mb-2">لا توجد مواد دراسية بعد</h2>
          <p className="text-muted-foreground max-w-md mx-auto mb-6">
            أضف أول مادة دراسية لتصنيف دوراتك وتقديم تجربة منظمة لطلابك.
          </p>
            ...
            <AlertDialogDescription>
              {deleting && deleting.courses_count > 0 ? (
                <>
                  ...
                  دورة. الرجاء إعادة تعيين المادة على تلك الدورات أولًا.
                </>
              ) : (
                <>
                  هل أنت متأكد من حذف "{deleting?.name}"؟ لا يمكن التراجع عن هذا
                  الإجراء.
                </>
              )}
            </AlertDialogDescription>
